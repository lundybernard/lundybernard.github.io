...
> **TLDR**:
> BatConf is thread-safe, well tested, and ready for your free-theaded applications
...
* Python its self protects us against read/write contention on variables


## Some areas for concern
"You're far too trusting," -Grand Moff Tarkin

While BatConf is conceputally and architecturally thread-safe,
...
I dont think we need to be overly concerned with users creating their own
...
What if we missed something in the design, and there's some unexpected concurrency issue?
...
gives us reasonable certanty that there are no major issues.
...
### 2. Real threadding tests
We added a new suite of free-threading test cases.
These tests utilize BatConf Configuration objects in multiple threadeds.
...
I was further assured that our pure-python package should be relatively safe,
...
These failed spectacularly! Generating some suprising errors, and even locking up the pytest process!
...
```
Wait... why is it looking for `SHELL`?  `git grep SHELL` confirms that doesnt even appear in our code base...
...
Running all of our test suites together takes about 1.06s, performance is not a concern.
Isolating test cases with mutex locks,
and refactoring the code to use more dependency injection for the sake of testing,
does not provide much value, and is unlikely to reveal thread-safety issues in the code its self.
