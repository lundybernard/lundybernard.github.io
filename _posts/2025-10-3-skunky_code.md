...
Sometimes we need to deal with sub-standard code in a release.
...
I dont want to come across as too negative here,
because there are very good reasons to to be flexible when it comes to code quality.
If there is an outage, getting back up and running is the top priority.
We do not want to block team mates or other teams by waiting on a perfect implementation.
...
- Mark it as TODO, or FIXME
    - Its Fast!
    - Its part of the code, so we see it when working on that part of the codebase
    - But Its invisible outside of the code, not part of our planning or issue tracking.
    - And Easily ignored and put off till 'later'

- Create a cleanup ticket
    - Its tracked and can be included in planning
    - It can be included in reports on tech-debt
    - But Its invisible in the code.
...
My preffered solution, is to create a new submodule named `skunkworks`
to contain any code which doesnt meet quality and testing standards.
...
that can arise when code fails catastrophicly. 
...
Everyone involved gets to share the responsability, not just the author or approver of a specific pull request.
It is an architectural, and managerial decision that the whole team makes together.

#### Cleanup!
Putting all the untrusted code in one place makes finding what to cleanup easy.
Moving code out of skunkworks can take precidence over all other cleanup and refactoring work.

Its a great place for Jr. Devs to start, tell them to "go clean out skunkworks".
...
refactor and clean up untill it meets standards, and then merge into code base.

Its easier to prioritize for cleanup.
